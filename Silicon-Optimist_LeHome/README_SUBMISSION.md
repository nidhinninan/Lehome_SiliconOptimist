...
| **Access token** | `<DOCKER_HUB_READ_ONLY_TOKEN_REDACTED>` |
| **Scope** | Read-only (pull only) |
| **Expires** | Jul 30, 2026 |

```bash
# One-liner login + pull (copy-paste ready)
docker login -u nninspaceexp --password-stdin <<< "<DOCKER_HUB_READ_ONLY_TOKEN_REDACTED>" && \
...
# <DOCKER_HUB_READ_ONLY_TOKEN_REDACTED>
...
- **Token:** `<DOCKER_HUB_READ_ONLY_TOKEN_REDACTED>`
