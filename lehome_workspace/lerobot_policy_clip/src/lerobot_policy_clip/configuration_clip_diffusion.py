...
@PreTrainedConfig.register_subclass("clip_diffusion")
@dataclass
class ClipDiffusionConfig(DiffusionConfig):
    """Configuration for Diffusion Policy with a frozen CLIP-ViT-B/16 vision backbone.

    Inherits all DiffusionConfig parameters. Key overrides:
        vision_backbone: HuggingFace model ID for the CLIP vision encoder.
        use_cls_token: If True, use pooler_output (CLS-based) as image embedding.
    ...
    vision_backbone: str = "openai/clip-vit-base-patch16"
    pretrained_backbone_weights: str | None = None  # Unused — HF weights loaded directly
    use_cls_token: bool = True

    # Disable the crop pipeline — CLIP handles its own resize internally
