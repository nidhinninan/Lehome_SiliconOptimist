#!/bin/bash
# run_2run_dino_sweep_trial.sh — Two-run DINO sweep + optional rclone upload to Google Drive.
#
# Same sweep order as run_2run_dino_sweep.sh (MAP+registers, then baseline), with:
...
#   chmod +x run_2run_dino_sweep_trial.sh
#   ENABLE_RCLONE_CHECKPOINT_SYNC=1 ./run_2run_dino_sweep_trial.sh
#
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  Two-run DINO sweep (trial) finished."
