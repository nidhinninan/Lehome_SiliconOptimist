...
#
# Principia VM + RTX 4070 Ti 16GB: repo under /data/lehome_workspace — HF_HOME/UV_CACHE_DIR on /data (see
# principia_vm_troubleshooting.md). When LEHOME_VM_PROFILE=principia or workspace is under /data/lehome_workspace,
# defaults target 4070 Ti VRAM (batch 12, workers 10) unless TOP_SHORT_GPU_PROFILE=a100.
# Optional INCLUDE_LEGACY_VM_LAST_PATHS=1 for /root/data/... checkpoint detection on other VM layouts.
#
...
#   TOP_SHORT_GPU_PROFILE=4070ti|a100  When 4070ti (default on Principia /data paths): VRAM-safe batch/workers + output
#     suffix _4070ti. Set to a100 on the same paths if this run targets A100-sized batches instead.
#
# Usage:
...
# Principia /data workspace + RTX 4070 Ti 16GB: MAP+registers is VRAM-heavy — default smaller batch unless profiling as A100.
TOP_SHORT_4070TI=false
if [[ "${TOP_SHORT_GPU_PROFILE:-4070ti}" == "4070ti" ]]; then
    if [[ "${LEHOME_VM_PROFILE:-}" == "principia" ]] || [[ "$WORKSPACE_DIR" == /data/lehome_workspace ]] || [[ "$WORKSPACE_DIR" == /data/lehome_workspace/* ]]; then
        TOP_SHORT_4070TI=true
    fi
fi
...
if [[ "$TOP_SHORT_4070TI" == true ]]; then
    JOB_NAME="${JOB_NAME:-DINOv2_MAP_Registers_dp_top_short_150k_phase1_4070ti}"
    OUTPUT="${OUTPUT:-outputs/train/dp_top_short_dino_map_registers_150k_4070ti}"
else
...
elif [[ "${TOP_SHORT_4070TI:-false}" == true ]]; then
    WORKERS=10
fi

echo ""
echo "================================================================"
echo "  dp_top_short DINO  TOP_SHORT_GPU_PROFILE=${TOP_SHORT_GPU_PROFILE:-4070ti}  4070ti_preset=${TOP_SHORT_4070TI}"
